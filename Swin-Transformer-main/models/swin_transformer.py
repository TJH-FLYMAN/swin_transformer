...
把序列转化为窗口
 输入：特征图（56，56，96）
 默认窗口大小为7  一共可以分成 8*8 的窗口
 输出特征图 （64，7，7，96）
 之前单位为序列，变换后为窗口（64个窗口）
'''
def window_partition(x, window_size):
    ...
    """
    B, H, W, C = x.shape
    # (2,8,7,8,7,96):指把 56*56 的patch按照7*7的窗口划分
...
'''
window partition函数是用于对张量划分窗口，指定窗口大小。将原本的张量从 N H W C, 划分成 num_windows× \times×B, window_size, window_size, C，
其中 num_windows = H× \times×W / window_size，即窗口的个数。
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = self.proj(x)  #全连接层
        x = self.proj_drop(x) # 还原成输入的形式[2，3136，96]
        return x
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.shift_size > 0:
            ...
            '''
            print( mask_windows.unsqueeze(1).shape)    torch.size([64,1,49])
            print( mask_windows.unsqueeze(2).shape)     torch.size([64,49,1])
            print(attn_mask.shape)                       torch.size([64,49,49])
            '''

            #attn_mask不为0的部分填充为 -100
    ...
    def forward(self, x):
        ...
        # cyclic shift
        # 在第一次是W-MSA，没有滑动窗口，所以self.shift_size > 0 =False
        if self.shift_size > 0:
...
然后拼接在一起作为一整个张量，最后展开。此时通道维度会变成原先的4倍（因为H,W各缩小2倍），
此时再通过一个全连接层再调整通道维度为原来的两倍
'''
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        print(self.proj(x).shape)                              torch.size([2,96,56,56])    224/4 =56
        print(self.proj(x).flatten(2).shape)                   torch.size([2,96,3136])   56*56=3136
        print(self.proj(x).flatten(2).transpose(1, 2).shape)   torch.size([2,3136,96])   
        '''
        x = self.proj(x).flatten(2).transpose(1, 2)  # B Ph*Pw C
        # x = torch.Size([2, 3136, 96])
        # 56*56 = 3136个patch=tokens
        # 每个patch或tokens的向量维度为96  96为实现设定好的embedding长度   patch_size也一样
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.pos_drop(x)  #self.pos_drop 一个dropout层
